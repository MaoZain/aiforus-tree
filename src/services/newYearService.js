...
class NewYearService {
  constructor() {
    this.templatePath = path.join(process.cwd(), 'src', 'templates', 'newyear.html');
  }
  ...
  async generate(data) {
    ...
    const outputFile = path.join(outputDir, 'newyear.html');

    try {
      // 1. Prepare Data
      const defaultText = '新年快乐！\n愿新的一年，带给你新的希望，新的梦想，新的开始。\n愿你在新的一年里，健康快乐，心想事成！';
      ...
      logger.info(`Generated New Year HTML for ID: ${id}`);

      // 5. Return URL
      return {
        id,
        url: `${config.publicUrlPrefix}${id}/newyear.html`
      };

    } catch (error) {
      logger.error('Error generating New Year HTML', error);
      // Clean up if failed
...
module.exports = new NewYearService();
